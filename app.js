...
document.getElementById('btn-to-fortune').addEventListener('click', () => showScreen('screen-fortune'));

// ── Fortune List ──
document.getElementById('btn-back-fortune').addEventListener('click', () => showScreen('screen-landing'));
...
// ── Fortune Back Buttons ──
document.getElementById('btn-back-toilet').addEventListener('click', () => showScreen('screen-fortune'));
document.getElementById('btn-back-egg').addEventListener('click', () => showScreen('screen-fortune'));
document.getElementById('btn-back-clover').addEventListener('click', () => showScreen('screen-fortune'));
...
document.getElementById('btn-fore-other').addEventListener('click', () => showScreen('screen-fortune'));
...
// ── Reset fortune test to initial state ──
...
function renderEggResult(surface, type) {
  ...
  if (type === 'perfect') {
    ...
    yolk(cx - 14, cy - 14, 28);
    const frag = makeEl('div', { position:'absolute', left:`${cx + 18}px`, top:`${cy - 22}px`, width:'10px', height:'7px', background:'#f0ebe3', borderRadius:'2px', transform:'rotate(28deg)' });
    frag.className = 'egg-shell-frag';
  }
}
...
function buildCloverSVG(leaves) {
  ...
  if (leaves === 0) {
    // stem only — already drawn
  } else if (leaves === -1) {
    // wilted: droopy 3-leaf
...
// ══════════════════════════════
// DECISION FLOW (existing)
// ══════════════════════════════
document.getElementById('btn-back-input').addEventListener('click', () => showScreen('screen-landing'));
document.getElementById('btn-back-method').addEventListener('click', () => showScreen('screen-input'));
